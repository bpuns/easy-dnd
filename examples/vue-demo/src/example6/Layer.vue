...
useDragListen({
  ...
  // drop一定比dragEnd先执行（原生行为）
  drop() {
    isDrop = true
    cloneDragDom.classList.add('reset-animation')
    cloneDragDom.style.transform = `translate(${targetRect.x}px, ${targetRect.y}px) scale(1)`
    cloneDragDom.style.width = `${startRect.width * scale}px`
    cloneDragDom.style.height = `${startRect.height * scale}px`
    cloneDragDom.ontransitionend = () => {
      // 还原样式
      layerDom.value!.innerHTML = ''
      dragDom.classList.remove('hide')
